...
public class CameraPreview extends TextureView {
    ...
    private Handler showPhotoHandler;
    ...
    public void setShowPhotoHandler(Handler showPhotoHandler) {
        ...
        private void process(CaptureResult result) {
            switch (mState) {
                ...
                case STATE_WAITING_LOCK: {
                    ...
                    } else if (afState == 0){//定焦摄像头 yuanye
                        Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                        if (aeState == null || aeState == CaptureResult.CONTROL_AE_STATE_CONVERGED) {
                            mState = STATE_PICTURE_TAKEN;
                            captureStillPicture();
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                Size largest = Collections.max(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)),
                        (Comparator<? super Size>) new CompareSizesByArea());
        ...
        try {
            ...
            CameraCaptureSession.CaptureCallback captureCallback
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               @NonNull CaptureRequest request,
                                               @NonNull TotalCaptureResult result) {
                    Toast.makeText(getContext(), "照片已保存", Toast.LENGTH_SHORT).show();
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile, showPhotoHandler));
        }
    };
    ...
    private static class ImageSaver implements Runnable {
        ...
        ImageSaver(Image image, File file, Handler handler) {
            mImage = image;
            mFile = file;
            mHandler = handler;
        }
